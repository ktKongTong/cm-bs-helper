...
//            val path = infoFilePath
            ...
            HashingSink.sha1(blackholeSink()).use { sink ->
                val paths = infoFile.toFile().source().buffer().use inner@{ source ->
                    ...
                    val paths = infoc.difficultyBeatmapSets.flatMap {bms->
        ...
        fun extractMapInfoFromDir(playlistAbsolutePath:String, file: File, playlistId:String): ExtractedMapInfo {
            ...
            val digest = HashingSink.sha1(blackholeSink()).use { sink ->
                val paths = file.source().buffer().use inner@{ source ->
                    source.readAll(sink)
                    source.readUtf8()
        ...
        fun extractMapInfoFromDirV2(mapPath:Path): IExtractedMapInfo {
            ...
            paths.map {pathInfo ->
                try {
                    ...
                    return IExtractedMapInfo.ErrorMapInfo(hash, mapId, mapPath, info,
