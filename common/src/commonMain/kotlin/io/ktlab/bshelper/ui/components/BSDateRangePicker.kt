...
//            dateFormatter = DatePickerFormatter("yyyy-MM-dd"),
                headline = {
                    Column {
                        Text(
                            text = "${state.selectedStartDateMillis?.let { millisToDateFormatString(it) } ?: "Start Date"} to ${millisToDateFormatString(state.selectedEndDateMillis) ?: "End Date"}",
            ...
            Row (Modifier.weight(1f,false).fillMaxWidth().padding(end = 16.dp),horizontalArrangement = Arrangement.End){
